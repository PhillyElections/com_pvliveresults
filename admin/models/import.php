...
class PvliveresultsModelImport extends PvliveresultsModel
{
    ...
    public function addNewCandidates()
    {
        ...
        $field = 'candidate';
        $this->_db->setQuery("SELECT DISTINCT " . $this->_db->nameQuote($field) . " as `name`, 1 as `published`, '" . $this->getNow() . "' as `created` ' from " . $this->_db->nameQuote($this->_table) . " WHERE " . $this->_db->nameQuote($field) . " NOT IN (SELECT `name` FROM " . $this->_db->nameQuote($table) . " )");
